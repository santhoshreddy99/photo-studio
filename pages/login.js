...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    // Sign in using the 'customer' provider
    const res = await signIn('customer', { redirect: false, email, password })
    if (res?.error) {
      setError(res.error)
      setLoading(false)
      return
    ...
    // On success, redirect to customer's gallery
    router.push(`/gallery/${encodeURIComponent(email)}`)
  }

  return (
    <div className="max-w-md mx-auto mt-20 bg-white p-8 rounded-lg shadow">
      <h1 className="text-2xl font-semibold mb-4">Customer sign in</h1>
