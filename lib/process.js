...
async function processPhoto(photo) {
  ...
  // Generate a small LQIP placeholder (20px width) and return as base64 data URL
  const tiny = await sharp(body).resize({ width: 20 }).blur(1).webp({ quality: 40 }).toBuffer()
  ...
  return { thumbKey, webpKey, placeholder }
}
