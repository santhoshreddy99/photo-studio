...
async function sendCredentialEmail({ to, name = 'Customer', tempPassword, appUrl = process.env.APP_URL || 'http://localhost:3000' }) {
  ...
  const from = process.env.SENDGRID_SENDER || 'no-reply@creativegraphics.example.com'
  const subject = 'Your Creative Graphics Account'

  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;color:#222;line-height:1.4">
      <p>Hi ${name},</p>
      <p>Your account has been created for the Creative Graphics gallery. Use the temporary password below to login, then change it.</p>
      ...
      <p style="color:#666;font-size:13px">If you did not expect this, please contact Creative Graphics.</p>
      <p style="margin-top:18px">— Creative Graphics</p>
    </div>
  `

  const text = `Hi ${name}\n\nYour temporary password: ${tempPassword}\n\nLogin: ${appUrl}/login\n\nIf you did not expect this, please contact Creative Graphics.`
